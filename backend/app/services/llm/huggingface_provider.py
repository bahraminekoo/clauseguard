...
class HuggingFaceLLMProvider(LLMProvider):
    """LLM provider that calls Hugging Face's OpenAI-compatible chat API."""
    ...
    def __init__(
        ...
        self.base_url = (base_url or settings.hf_llm_base_url).rstrip("/")
        self.model = model or settings.hf_llm_model

        if not self.api_key:
            raise ValueError("HF_API_KEY is required for HuggingFaceLLMProvider")
        ...
        async with httpx.AsyncClient(timeout=120.0) as client:
            for attempt in range(max_retries):
                ...
                if resp.status_code == 429:
                    ...
                    logger.warning("HuggingFace 429 rate limit, retrying in %.1fs (attempt %d/%d)", wait, attempt + 1, max_retries)
                    await asyncio.sleep(wait)
                    continue
                if resp.status_code == 503:
                    wait = backoff * (2 ** attempt)
                    logger.warning("HuggingFace 503 model loading, retrying in %.1fs (attempt %d/%d)", wait, attempt + 1, max_retries)
                    await asyncio.sleep(wait)
                    continue
                ...
                if not choices:
                    raise ValueError("No choices in HuggingFace response")
                content = choices[0].get("message", {}).get("content")
                if not isinstance(content, str):
                    raise ValueError("Missing content in HuggingFace response")
                # Validate JSON
                json.loads(content)
                return content
        raise httpx.HTTPStatusError(
            f"HuggingFace API failed after {max_retries} retries (status {resp.status_code})",
